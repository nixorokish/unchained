...
func (h *History) doRequest2(txState *TxState) (*TxSearchResponse, error) {
	for {
		var res *TxSearchResponse
		var resErr *RPCErrorResponse
		...
		body := `{
			"network": "cosmos",
			"account": [
			  "cosmos1fx4jwv3aalxqwmrpymn34l582lnehr3eqwuz9e"
			],
			"limit": 2
		  }`

		_, err := h.txSearch.R().SetResult(&res).SetError(&resErr).SetBody(body).Post("/transactions_search")
...
func (h *History) fetch2() (*TxHistory, error) {
	...
	logger.Info("FETCH2")
	logger.Info("FETCH2!")
	res, err := h.doRequest2(h.txs)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get send tx history")
	}
	h.txs.txs = res.Result.Txs

	logger.Infof("h is %+v", *h.txs)
	...
	for len(txs) < h.pageSize {
		// fetch more send transactions if we have run out and more are available
		if len(h.txs.txs) == 0 && h.txs.hasMore {
			if err = h.fetchMore2(h.send); err != nil {
				return nil, errors.Wrap(err, "failed to fetch more send txs")
			}
		}
		...
		// get next transaction and remove from the txs set
		var next TxSearchResponseResultTxs
		next = h.send.txs[0]
		h.txs.txs = h.send.txs[1:]
		h.txs.lastID = *next.Hash

		logger.Infof("next is %+v", next)
		cosmosTx, signingTx, err := DecodeTx(*h.encoding, *next.Tx)
		if err != nil {
			logger.Errorf("failed to decode tx: %s: %s", *next.Hash, err.Error())
			continue
		...
		logger.Infof("cosmosTx is %+v", cosmosTx)
...
func (h *History) fetchMore2(txState *TxState) error {
	txState.page++
